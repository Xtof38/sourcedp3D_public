...
list='/home/chris/sourcedp3D/dp3D_examples/crush_cluster/input_dp3D'
#case $option in
#  add_line ) 
...
# replaces only between line0 and $long_line
    n=$(grep -n "$kw_to_modify" $fich | awk '{print $1}' | sed 's/:\#//')
    if [ "$n" != "" -a "$nn" = "" ];then
      let n=n+1
echo "n $n"
      nn=$(awk 'NR=="'$n'"' $fich | grep -n "\#" | awk '{print $1}' | sed 's/:\#//')
      if [ "$nn" = 1 ];then
# delete the line
        awk -v n="$n" 'NR == n {next} {print}' $fich
