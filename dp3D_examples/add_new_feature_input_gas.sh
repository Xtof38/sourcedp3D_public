...
for fich in $list ; do
  ...
  if [ $file2treat = "0" ];then
    ...
    if [ "$linetorm" != "" ];then
      ...
      grep -n "# prop. of class (particle and aggregate" $fich | cut -f1 -d: > listtorm.tmp
      ntrm=$(grep -c . listtorm.tmp)
      for ((  ii = 1 ;  ii <= $ntrm;  ii++  )); do
        linetorm=$(tail -1 listtorm.tmp)
        let "lp2 = linetorm + 2"
        let "lp4 = linetorm + 4"
        sed  -i ''$lp2','$lp4'd'  $fich 
        sed -i '$ d' listtorm.tmp
      done
    fi
  fi
done
...
old_char="# number= mat= name= particle_size= deviation= motif="
